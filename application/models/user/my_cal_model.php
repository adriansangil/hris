...
class My_cal_model extends CI_Model{
	...
	public function __construct()
	{
		$this->conf = array(
			'show_next_prev' =>true,
			'next_prev_url' => base_url().'index.php/my_dashboard/my_calendar',
			'day_type' => 'short'
			);
	...
	function get_pending_leave($user_id){
		$this->db->select('*');
		$this->db->from('leave');
	...
	function get_recent_leave($user_id){
		$this->db->select('*');
		$this->db->from('leave');
		$this->db->join('employee_information', 'employee_information.employee_id = leave.employee_id','inner');
		$this->db->join('leave_type', 'leave_type.leave_type_id = leave.leave_type_id','inner');
		$this->db->where('leave.notification','1');
		$this->db->where('leave.status !=','Pending');
		$this->db->where('employee_information.employee_id',$user_id);
		$this->db->order_by('leave.leave_id','desc'); 
		$query = $this->db->get();
	...
	function get_recent_medical($user_id){
		$this->db->select('*');
		$this->db->from('medical');
		$this->db->join('employee_information', 'employee_information.employee_id = medical.employee_id','inner');
		$this->db->where('medical.notification','1');
		$this->db->where('medical.status !=','Pending');
		$this->db->where('employee_information.employee_id',$user_id);
		$this->db->order_by('medical.medical_id','desc'); 
		$query = $this->db->get();
